...
//
// It does not change the node's desired run state: a stopped node
// ("tailscale down") remains stopped, though the login can still be
// completed, and a running node stays running while the login proceeds
// in the background. If the login is completed as a different user or
// node identity than the current profile's, the current profile is
