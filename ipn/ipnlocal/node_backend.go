...
func (nb *nodeBackend) nodeByFQDNLocked(fqdn dnsname.FQDN) (_ tailcfg.NodeView, ok bool) {
	nid, ok := nb.nodeByName[strings.TrimSuffix(string(fqdn), ".")]
	if !ok {
		return tailcfg.NodeView{}, false
	}
...
func dnsConfigForNetmap(nm *netmap.NetworkMap, peers map[tailcfg.NodeID]tailcfg.NodeView, prefs ipn.PrefsView, selfExpired bool, logf logger.Logf, goos string) *dns.Config {
	...
	if nm.DNS.Proxied { // actually means "enable MagicDNS"
		...
		// they were listed in dcfg.Hosts. The existence check short
		// circuits, so it is O(1) in practice.
		haveNodeRecords := nm.SelfName() != "" && nm.GetAddresses().Len() > 0
		if !haveNodeRecords {
			for _, peer := range peers {
				if peer.Name() != "" && peer.Addresses().Len() > 0 {
					haveNodeRecords = true
					break
				}
			}
		}
		dcfg.MagicDNSHostsUnrouted = haveNodeRecords
	}
