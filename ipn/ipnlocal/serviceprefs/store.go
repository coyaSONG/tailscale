...
type Store interface {
	...
	// the result back under the same key. Running the read-modify-write under a single lock lets
	// concurrent callers merge different fields of the same pref without losing each other's
...
// result back under the same key. It uses a write lock to ensure thread safety. If the profile ID
