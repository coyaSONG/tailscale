...
func TestUpdateForProfileMergesConcurrently(t *testing.T) {
	// Each goroutine updates a different field of the same key. Because UpdateForProfile runs the
	// read-modify-write under one lock, no update should clobber another's field.
	ctx := t.Context()
	store := NewInMemoryStore(0, nil)
	pid := ipn.ProfileID("pid")
