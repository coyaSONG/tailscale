...
func TestNodeBackendMagicDNSHosts(t *testing.T) {
	...
	if handled := nb.UpdateNetmapDelta([]netmap.NodeMutation{netmap.MakeNodeMutationRemove(2)}); !handled {
		t.Fatal("UpdateNetmapDelta not handled")
	}
	...
	if handled := nb.UpdateNetmapDelta([]netmap.NodeMutation{netmap.NodeMutationUpsert{Node: p3.View()}}); !handled {
		t.Fatal("UpdateNetmapDelta not handled")
	}
