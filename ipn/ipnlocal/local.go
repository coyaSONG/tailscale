...
// first call. It may block on disk I/O the first time, so callers that hold [LocalBackend.mu]
// should warm it beforehand.
...
// ServicePrefs returns the service prefs for the current profile. When there is no current
// profile (for example, when logged out), it returns an empty map rather than an error, since
// that's a normal state with no prefs to report.
...
// service prefs, records [ipn.ServicePref.LastUsed], and returns the updated prefs for the
// current profile.
