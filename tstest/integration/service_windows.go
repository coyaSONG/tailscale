...
func (n *TestNode) startWindowsServiceDaemon() *Daemon {
	...
	// These tests uninstall the service and wipe its state; refuse to run if a
	// Tailscale service or state dir already exists, to avoid breaking a real install.
	if serviceExists(t) || tailscaleStateExists() {
		t.Skip("existing Tailscale service or state found; run only on a disposable/CI machine")
	}
...
// tailscaleStateExists reports whether the service state dir is already present.
func tailscaleStateExists() bool {
	_, err := os.Stat(serviceStateDir())
	return err == nil
}
